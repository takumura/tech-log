...
(function (root, factory) {
  ...
  return function (lunr) {
    ...
    // Directly inject lunr.stemmer.support.js code - start
    lunr.stemmerSupport = {
    ...
    // Directly inject tinyseg.js code - start
    ...
    // original lunr.ja.js code from here
    /*
    ...
    lunr.ja = function () {
      this.pipeline.reset();
      this.pipeline.add(lunr.ja.trimmer, lunr.ja.stopWordFilter, lunr.ja.stemmer);
    ...
    lunr.ja.tokenizer = function (obj) {
      ...
      for (sliceEnd = 0, sliceStart = 0; sliceEnd <= len; sliceEnd++) {
        ...
        if (char.match(/\s/) || sliceEnd == len) {
          if (sliceLength > 0) {
            segs = segmenter.segment(str.slice(sliceStart, sliceEnd)).filter(function (token) {
              return !!token;
            });
